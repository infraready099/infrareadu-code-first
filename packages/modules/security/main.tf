...
# ─── GITHUB ACTIONS OIDC — ZERO LONG-LIVED CREDENTIALS ───────────────────────
# Enabled when enable_github_deploy_role = true.
# Creates an OIDC provider + least-privilege deploy role so GitHub Actions can
# push Docker images to ECR and update ECS (or sync to S3/CloudFront for static
# sites) without ANY long-lived AWS credentials stored in GitHub Secrets.
#
# Security model:
#   - Trust is scoped to a single repo (github_repo_slug) — no other repo can assume this role
#   - Tokens are short-lived (15 min) and automatically rotated by GitHub
...
resource "aws_iam_openid_connect_provider" "github_actions" {
  ...
  # Both current thumbprints — GitHub has rotated once before; include both for resilience
  thumbprint_list = [
    ...
    Statement = [
      {
        ...
        Condition = {
          ...
          StringLike = {
            # Scoped to this specific repo only — no other repo can assume this role
            "token.actions.githubusercontent.com:sub" = "repo:${var.github_repo_slug}:*"
          }
        }
      }
    ]
...
# ECS deploy policy — push image to ECR + trigger ECS redeployment
resource "aws_iam_role_policy" "github_deploy_ecs" {
  count = var.enable_github_deploy_role && var.deployment_target == "ecs" ? 1 : 0
  name  = "ecs-deploy"
...
# Static site deploy policy — sync to S3 + CloudFront invalidation
resource "aws_iam_role_policy" "github_deploy_static" {
  count = var.enable_github_deploy_role && var.deployment_target == "static" ? 1 : 0
  name  = "static-deploy"
  role  = aws_iam_role.github_deploy[0].id
